...
def make_bin_wall_cfgs(bin_size: float, wall_thickness: float = 0.02,
                       wall_height: float = 0.10) -> list:
    """Build 3 bin-wall cuboids (back + 2 sides, exit opens toward -X).

    Bin spans EW: [0, bin_size] → MPPI x: [0.35, 0.35+bin_size]
              NS: [0, bin_size] → MPPI y: [0.075, 0.075+bin_size]
    Exit is at EW=0 (MPPI x = 0.35, the -X side — no wall there).
    Back wall at EW=bin_size (high-X); side walls at NS=0 and NS=bin_size.
    """
    T = wall_thickness
    H = wall_height
    x0 = 0.35    # MPPI x at EW=0
    y0 = 0.075   # MPPI y at NS=0
    z0 = 1.194   # table top z (table center 1.159 + half-height 0.035)
    z_c = z0 + H / 2

    wall_material = sim_utils.PreviewSurfaceCfg(diffuse_color=(0.65, 0.55, 0.45), opacity=0.7)
...
def make_static_cfgs(stand_urdf: str, bin_size: float | None = None,
                     wall_thickness: float = 0.02) -> list:
    ...
    if bin_size is not None:
        cfgs += make_bin_wall_cfgs(bin_size, wall_thickness)
    return cfgs

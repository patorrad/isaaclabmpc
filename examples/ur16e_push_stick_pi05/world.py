"""Isaac Lab world runner for UR16e push-stick task with pi0.5-base.
...
------------
  ...
  Terminal 2 (isaaclab conda env):
      ...
      python examples/ur16e_push_stick_pi05/world.py [--headless]
...
Policy action output:
    (10, 6) float32 — predicted delta joint positions over 10-step horizon
"""
...
parser = argparse.ArgumentParser(description="UR push-stick world (pi0.5-base)")
...
class Pi05Policy:
    ...
    def infer(self, base_img: np.ndarray, wrist_img: np.ndarray,
        ...
        Returns:
            (10, 6) float32 — delta joint positions
        """
        obs = {
            "base_rgb":  base_img,
            "wrist_rgb": wrist_img,
...
def main():
    ...
    print(f"[world] robot = {robot_name}")
    ...
    print(f"[world] {cfg.n_steps} steps | action_execute={cfg.pi05.action_execute} | "
          f"scale={cfg.pi05.action_scale}")
    print(f"[world] object {PUSH_OBJECT_INIT_POS} → goal {GOAL_MARKER_POS}")
